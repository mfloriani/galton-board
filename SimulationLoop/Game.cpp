...
Game::Game(HDC hdc) : m_hdc(hdc), m_previousTime(0)
{
	...
	m_physicsSys->AddRigidBody(
		CreateOBB(
			math::Vector3D(0.0f, -45.f, 0.f), 
			math::Vector3D(0, 0, 0), 
			0.0f, 
			math::Vector3D(22.0f, 1.0f, 1.0f), 
			math::Vector3D(0.f, 0.f, 0.f),
			BLACK
		)
	);
...
void Game::AddBalls()
{
	for (int i = 0; i < BALL_TOTAL; ++i)
	{
		...
		m_physicsSys->AddRigidBody(CreateSphere(math::Vector3D(x, 20 + y, 0), math::Vector3D(0, 0, 0), BALL_MASS, BALL_SIZE, WHITE));
	}
}